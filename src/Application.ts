    ...
    export class Application {
        ...
        public addListHandler(e: Event): void {
            ...
            const callback = async () => {
                const addCallback = async (e: Event) => {
                    ...
                    const res = await this._trebis.createList(name);
                    if (res) {
        ...
        public runScriptHandler(e: Event): void {
            ...
            const callback = async () => {
                ...
                if (this._trebis.thisListId) {
                    await this._trebis.initLabels();
                    // Обновляем карточки
                    const count = await this._trebis.updateCard();
                    if (count !== null) {
        ...
        public removeHandler(e: Event) {
            ...
            if (confirm('Уверены что хотите удалить старые карточки, при этом останутся первые 30?')) {
        ...
        public statisticHandler(event: MouseEvent, isFull: boolean = false) {
            ...
            TrelloUI.openModal(this._getStatisticContentTemplate(isFull), 'Получение статистики');
        ...
        public statisticOrgHandler(event: MouseEvent) {
            ...
            const trebisServerData: HTMLElement = document.querySelector(`.${this.STAT_SERVER_LABEL}`);
            if (trebisServerData) {
        ...
        protected _getStatisticContentTemplate(isFull: boolean = false): string {
            ...
            let res = '<div class="window-main-col" style="margin: 12px 40px 8px 56px;width:calc(100% - 100px)"><div style="display: flex">' +
                `<div><label for="${this.STAT_DATE_START}">с</label>` +
                `<input type="date" id="${this.STAT_DATE_START}" style="width: 100%" value="${utils.revertDate(dateStart)}"></div>` +
                `<div style="margin-left: 15px"><label for="${this.STAT_DATE_END}">до</label>` +
                ...
                `<p id="${this.STAT_DATE_MONTH}" style="${styleLink}">Текущий месяц</p>` +
                `<p id="${this.STAT_DATE_OLD_MONTH}" style="${styleLink}">Прошлый месяц</p>` +
                `<p id="${this.STAT_DATE_ALL}" style="${styleLink}">За все время</p>` +
                `<p id="${this.STAT_DATE_COMPARISON}" style="${styleLink}">Сравнить ${this._getMonthText(month)} и ${this._getMonthText(month - 1)}</p>` +
                '</div></div>' +
                `<div><button class="nch-button--primary ${this.STAT_BTN}">Построить</button></div>`;

            if (isFull) {
                const year = (new Date()).getFullYear();
                res += `<div style="display: flex" class="${this.STAT_SERVER_LABEL}">`;
        ...
        protected _getStatisticServerResultTemplate(serverData: IServerApiRequestRes, year: string): string {
            ...
            for (const boardName in serverDatas) {
                if (serverDatas.hasOwnProperty(boardName)) {
                    res += `<div><h3>Информация по доске: <u>${boardName}</u></h3><table>`;
                    ...
                    res += `<tr><td colspan="5">Итог за ${year}г.</td></tr>`;
                    const data: ITrebisStatistic = serverData.total[boardName];
                    res += this._getStatisticResultTemplate(data);
                    res += '</table></div>';
                }
            }
        ...
        protected _comparisonEqual(old: ITrebisStatistic, current: ITrebisStatistic): ITrebisStatisticText {
            ...
            if (old.red < current.red) {
                ...
                red = el(old.red - current.red, 'green', 'Хорошо! Не сделанных задач стало меньше!');
            }
        ...
        public createButtons() {
            if (!document.getElementById(Application.SELECTOR)) {
                ...
                innerHtml += TrelloUI.getHeaderButton({
                    id: runScriptName,
                    title: 'Нажмите для автоматического создания списка, и переноса не выполненных задач.',
                ...
                if (memberMenu) {
                    ...
                    if (this.ADMIN_USERS.includes(userName)) {
                        ...
                        innerHtml += TrelloUI.getHeaderButton({
                            ...
                            label: 'Удаление старых карточек',
                            icon: 'trash'
                        });
                    }
